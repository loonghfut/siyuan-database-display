...
export function addDisplayFormatPanel(
    ...
    addPanel("display-format", JSON.stringify({ dateFormat: "YYYY-MM-DD", includeTime: false, checkboxStyle: "icon", maxDisplayLength: 30, showFieldNames: false, listFontSize: 12, listMultiColumn: true, listLayoutStyle: "grid", listItemStyle: "plain", cardEnabled: true, editTrigger: "click", layout: "below" }), text.format.title, text.format.description, (value, commit) => {
        const state = parseObject<{ dateFormat?: string; includeTime?: boolean; checkboxStyle?: string; maxDisplayLength?: number; showFieldNames?: boolean; listFontSize?: number; listMultiColumn?: boolean; listLayoutStyle?: string; listItemStyle?: string; cardEnabled?: boolean; editTrigger?: string; layout?: string }>(value, {});
        ...
        listSettings.append(listHint, listLayoutStyleRow, createLabel(text.format.listItemStyle, listItemStyle), createLabel(text.format.fontSize, fontSize), multiColumnRow);
