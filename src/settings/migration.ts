...
export function migrateLegacySettings(settings: SettingUtils, saved: unknown): boolean {
    ...
    migrate("display-format", { dateFormat: data["date-format"] || "YYYY-MM-DD", includeTime: Boolean(data["include-time"]), checkboxStyle: data["checkbox-style"] || "emoji", maxDisplayLength: data["max-display-length"] || 30 });
